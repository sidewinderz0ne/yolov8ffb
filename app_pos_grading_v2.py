...
class Frame2(tk.Frame):
    ...
    def validate_divisi(self):
        return bool(self.divisi_entry.get().strip())
    ...
    def save_offline_log(self, tiket, plat, driver, unit, divisi, blok, bunches, status, entry_fields):
        ...
        result = f"{tiket};{plat};{driver};{unit};{divisi};{blok};{bunches};{ownership};{current_date.strftime('%Y-%m-%d %H:%M:%S')};None"
        ...
        messagebox.showinfo("Success", f"Data Tiket {tiket} Sukses Tersimpan di Table Utama!")  
...
class ConfirmationDialog(tk.Toplevel):
    def __init__(self, parent, title, message):
        ...
        tk.Label(self, text=message, wraplength=300).pack(padx=20, pady=20)
        ...
        yes_button = ttk.Button(button_frame, text="Yes", style='TButton', command=self.on_yes)
        yes_button.pack(side=tk.LEFT, padx=10)

        no_button = ttk.Button(button_frame, text="No", style='TButton', command=self.on_no)
