...
default_model = 'best'
model_files = glob.glob(os.path.join(current_folder, 'model', str(default_model) + '.pt'))
...
yolo_model_str = None
window_yolo_model = None

if hasattr(opt, 'yolo_model') and opt.yolo_model != parser.get_default('yolo_model'):
    if not os.path.isabs(yolo_model):
        # Append the current folder to the provided yolo_model
        yolo_model_str = os.path.join(current_folder, 'model', yolo_model)
    # Ensure the model file has the correct extension (.pt)
    if not yolo_model_str.endswith('.pt'):
        yolo_model_str += '.pt'

    if os.path.exists(yolo_model_str):
        window_yolo_model = yolo_model
        model = YOLO(yolo_model_str)
    else:
        window_yolo_model = default_model
        model = YOLO(default_model_source)
else:
    window_yolo_model = default_model
    model = YOLO(default_model_source)
...
window = "Yolov8 "+str(imgsz) + " CONF-" + str(conf_thres) + " IOU-" +  str(iou_thres) + ' YOLO MODEL-' + '/model/' +str(window_yolo_model) + '.pt'
...
try:
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            # for r in results[0].boxes.xywh.tolist():
            #     wideArea = int(r[2]) * int(r[3])
            #     print(wideArea)
            #     if wideArea > min_area:
            #         annotated_frame = results[0].plot(conf=False)
            ...
            # printBoxes = []
            # printBoxes[0].append(results[0])
            wideArea = 0

            for idx, (box, track_id, cl) in enumerate(zip(boxes, track_ids, clss)):
                x, y, w, h = box
                
                wideArea = int(w) * int(h)

                # if wideArea > min_area:
                #     printBoxes[0].pop(idx)
                ...
                if wideArea < max_area and int(cl) < (int(len(class_count)-1)) and wideArea > min_area:
                    ...
                    cv2.rectangle(annotated_frame, (rect_x, rect_y), (rect_x + rect_width, rect_y + rect_height), (255, 255, 255), -1)
                    cv2.putText(annotated_frame, text, (int(x), int(y)), font, fontRipeness, (0, 0, 0), 2)
